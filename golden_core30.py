...
def create_stock_data(db_file_name, code_list, start_date, end_date):
    ...
    for code in code_list:
        unit = conn.execute('SELECT unit FROM brands WHERE code=?', (code,)).fetchone()[0]
        prices = pd.read_sql('SELECT date, open, close FROM prices WHERE code=? AND date BETWEEN ? AND ? ORDER BY date', conn, params=(code, start_date, end_date), parse_dates=('date',), index_col='date')
        stocks[code] = {'unit': unit, 'prices': prices.reindex(tse_index, method='ffill')}
    return stocks
