...
def create_divide_union_table():
    ...
    with conn:
        sql = """
        CREATE TABLE if not exists divide_union_data(code TEXT, date_of_right_allotment TEXT, before REAL, after REAL, PRIMARY KEY(code, date_of_right_allotment))
        """
        conn.execute(sql)
...
def all_csv_file_to_divide_union_table(db_file_name, csv_file_dir):
    ...
    with conn:
        sql = """
        INSERT INTO divide_union_data (code, date_of_right_allotment, before, after)
        VALUES(?,?,?,?)
        """
